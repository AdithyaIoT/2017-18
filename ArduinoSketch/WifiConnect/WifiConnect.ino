...
char server[] = "77b1bcaf.ngrok.io";    // name address for Google (using DNS)
...
void setup() {
  ...
  if (client.connect(server, 80)) {
    Serial.println("connected to server");
    int bpm=150;
    //client.println("GET /AdiHealthBand/Index.jsp?bandParam=Publish&param1=pulse&param2=100 HTTP/1.1"); 
    client.println("GET /AdiHealthBand/Index.jsp?bandParam=Publish&param1=pulse&param2=" + String(bpm) + " HTTP/1.1");
    //client.println("GET /AdiHealthBand/Index1.html HTTP/1.1"); 
